...
class Manager:
    ...
    def run(self, mission: str, resume: bool = False) -> ProjectMemory:
        ...
        self.memory.diagnostics["release_bundle_ready"] = bool(release_report.ready and release_quality_ok)
        ...
        if release_report.ready:
            release_hash = self.memory.diagnostics.get("release_bundle_sha256") or hashlib.sha256(json.dumps(release_report.artifacts, sort_keys=True).encode()).hexdigest()
        ...
        self.memory.diagnostics["quality_passed"] = release_quality_ok
        completion = {
            "status": self.memory.status,
            "mission": self.memory.mission,
            "quality_passed": release_quality_ok,
