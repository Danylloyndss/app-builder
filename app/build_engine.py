    ...
    def _javascript(features: list[str] | None = None) -> str:
        ...
        if "storage" in features:
            blocks += [
                ...
                "  };",
                "  const contract = await fetch(apiBase + '/api_contract.json').then(response => response.json());",
                "  const primary = contract.resources[contract.primary_resource];",
                "  const endpoint = primary?.GET || contract.compatibility?.records || '/api/records';",
                ...
                "    try { renderRecords(await api(endpoint)); } catch (_) {",
                ...
                "    try { await api(primary?.POST || endpoint, {method: 'POST', body: JSON.stringify(payload)}); await loadRecords(); if (status) status.textContent = 'Saved to server'; }",
