...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send_csv(self):
        employee,company,date_from,date_to=self._filters() if hasattr(self, "_filters") else (None,None,None,None); output=io.StringIO(); writer=csv.writer(output); writer.writerow(["ID","Employé","Entreprise","Date","Lieu","Début","Pause (min)","Fin","Total","Recado","Signature","Pièces jointes"])
        try: export_rows=TIMEPRO.history(employee,company,date_from,date_to)
        except TypeError: export_rows=TIMEPRO.history()
        for r in export_rows: writer.writerow([r["id"],r["employee"],r.get("company",""),r["work_date"],r["location"],r["start_time"],r["pause_minutes"],r["end_time"],f'{int(r["total_minutes"])//60}h {int(r["total_minutes"])%60:02d}min',r["note"],"oui" if r.get("has_signature") else "non",len(r.get("attachments",[]))])
